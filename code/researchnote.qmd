...
data$edu <- as.character(Recode(data$edu,"1:3 = 1;4:5 = 2;6:7 = 3;8:9 = 4"))
data$party <- as.factor(Recode(data$party,"2:4 = 0"))
data$rural <- as.factor(Recode(data$rural,"c(1,3) = 0;2 = 1"))
data$race <- as.factor(Recode(data$race,"2:5 = 0"))
data$marriage <- as.factor(Recode(data$marriage,"2 = 1;3:6 = 0;1 = 0"))
data$gender <- as.factor(2 - data$gender)
...
table1 <- data %>%
  select(.,treat,age,gender,edu,party,marriage,incomeFam_level,occ,newspol,internetuse,nationalism) %>%
  datasummary_balance(
  ~ treat,
  data = .,
...
data_over <- data %>%
  filter(.,cate_over4 == 1) %>%
  select(.,treat,demSuit,demSolve,dempositive,demStrong) 
